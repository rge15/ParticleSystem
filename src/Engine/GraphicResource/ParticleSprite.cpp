...
//-----------------------------------------------------------------------------
	...
	) const noexcept
	{
		...
		__m128i alphaMask = _mm_set_epi32( 0xFFFF0000, 0x00000000, 0xFFFF0000, 0x00000000 );
		// __m128i alphaMask = _mm_set1_epi64( 0xFFFF000000000000 );

		for( i = iStart ; i < iEnd ; i++)
		{
			for(__m128i* jStart = reinterpret_cast<__m128i*>(_data[i * _width]),
				*jEnd = reinterpret_cast<__m128i*>(_data[i * _width + _width - 1]); 
				jStart < jEnd; i++ )
			{
				__m128i sprColorHi = _mm_unpackhi_epi8( *jStart, zero );
				__m128i sprColorLo = _mm_unpackhi_epi8( *jStart, zero );
			
				// 		srcA = uint16_t(srcA * pColor.A) >> 8;
				// 		srcR = uint16_t(srcR * pColor.R) >> 8;
				// 		srcG = uint16_t(srcG * pColor.G) >> 8;
				// 		srcB = uint16_t(srcB * pColor.B) >> 8;
				__m128i particleColorHi = _mm_mulhi_epu16( sprColorHi, srcColor );
				__m128i particleColorLo = _mm_mulhi_epu16( sprColorLo, srcColor );

				__m128i particleAlphaHiValue = _mm_and_si128( alphaMask, particleColorHi ); //(A1)(0)(0)(0)(A2)(0)(0)(0)
				__m128i particleAlphaLoValue = _mm_and_si128( alphaMask, particleColorLo ); //(A3)(0)(0)(0)(A4)(0)(0)(0)
				...
				// 		srcG = uint16_t(srcG * srcA) >> 8;
				// 		srcB = uint16_t(srcB * srcA) >> 8;
				particleColorHi = _mm_mulhi_epu16( particleColorHi, particleAlphaHi );
				particleColorLo = _mm_mulhi_epu16( particleColorLo, particleAlphaLo );
